@extends('layouts.app')
@section('title',"Evenement du BDE Cesi Lyon")
@section('meta-description',"Toutes les informations d'un évenement organisé par le BDE Cesi Lyon")@section('content')
<!--Creation of the event area-->
...
<div class="text-center mt-2 mb-2">
    ...
    @if(Auth::user() !== null)
        ...
        <a href="/events/{{$event->id}}/export" class="btn btn-cesi">Exporter la liste des participants</a>          
        @endif
    @endif
    ...
    @if(isset($sub))
        @if($event->start_date > date("Y-m-d"))
            ...
            @endif
        @else
            <!--Allow the possibility of import fotos-->
            @if($sub == 1)
...
@if(isset($sub))
    ...
    <div class="media mt-5">
        <img class="d-flex rounded-circle avatar z-depth-1-half mr-3" src="{{Auth::user()->image->path}}" alt="Avatar {{Auth::user()->first_name}} {{Auth::user()->last_name}}">
